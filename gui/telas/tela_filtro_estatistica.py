...
from core.despesas_filtrar_core import filtrar_anos_disponiveis,filtrar_ano_selecionado
from core.despesas_filtrar_core import filtrar_meses_disponiveis,filtrar_mes_selecionado
...
class TelaFiltroEstatistica(ttk.Frame):
    def __init__(self,parent,controller):
        ...
        self.anos = filtrar_anos_disponiveis()  # chamou funcao do do CORE
        self.anos_str = [str(ano) for ano in self.anos]  # recebeu o resultado da funcao acima e converteu em STRING
        self.meses = filtrar_meses_disponiveis()
        self.meses_disp = [nome for nome, numero in MESES.items() if numero in self.meses]
    ...
    def criar_widgets(self):
        ...
        ttk.Label(self.frame_formulario, text="Mês").grid(row=0, column=0, pady=PADY_CAMPO, padx=PADX_LABEL,sticky="e")

        self.data_mes = tk.StringVar()
        self.combo_mes = ttk.Combobox(
            self.frame_formulario,
            textvariable=self.data_mes,
            values=self.meses_disp,
            state="readonly"
        )
        self.combo_mes.bind("<<ComboboxSelected>>",self.ao_selecionar_mes)
        ...
        self.combo_ano.bind("<<ComboboxSelected>>",self.ao_selecionar_ano)
    ...
    def ao_selecionar_mes(self,event = None):
        mes_nome = self.data_mes.get()
        ...
        despesas_filtradas = filtrar_mes_selecionado(mes)
        anos_disponiveis = filtrar_anos_disponiveis(despesas_filtradas)
        self.combo_ano["values"] = [str(a) for a in anos_disponiveis]


    def ao_selecionar_ano(self,event = None):
        ano = self.data_ano.get()
        despesas_filtradas = filtrar_ano_selecionado(ano)
        meses_disponiveis = filtrar_meses_disponiveis(despesas_filtradas)
        meses_nomes = [nome for nome, numero in MESES.items() if numero in meses_disponiveis]
        self.combo_mes["values"] = meses_nomes
